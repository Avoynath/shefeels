...
function Bullet({ children }: PropsWithChildren) {
  ...
  return (
    <li className={`pl-2 text-sm leading-relaxed ${
      isDark ? "${isDark ? "text-white/70" : "text-gray-700"}" : "text-gray-700"
    }`}>{children}</li>
  );
}
...
function BodyTextCompact({ children }: PropsWithChildren) {
  const { theme } = useTheme();
  const isDark = theme === "dark";
...
const sections: Section[] = [
  {
    ...
    body: (
      <div className="text-sm ${isDark ? "text-white/70" : "text-gray-700"} leading-relaxed space-y-3">
        <p>
  ...
  },
  {
    ...
    body: (
      <ul className="list-disc pl-5 space-y-1 text-sm ${isDark ? "text-white/70" : "text-gray-700"} leading-relaxed">
  ...
  },
  {
    ...
    body: (
      <p className="text-sm ${isDark ? "text-white/70" : "text-gray-700"} leading-relaxed">
